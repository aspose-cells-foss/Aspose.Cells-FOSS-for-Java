...
/**
 * JUnit 5 tests for Workbook/Worksheet Settings API 鈥?WB-* test cases.
 */
class WorkbookTest {
    ...
    /**
     * Verifies that comment parts get an explicit Override in [Content_Types].xml 鈥?
    ...
    // =========================================================================
    // 4.12 Integration: Write Full Workbook Configuration 鈫?Verify via API and POI
    // =========================================================================
    ...
    void WB_B0_to_WB_BF_integrationFullWorkbook() throws Exception {
        try (TemporaryDirectory tempDir = new TemporaryDirectory("WorkbookTest")) {
            ...
            try (Workbook wb = new Workbook()) {
                ...
                // WB-B8: Row hidden 鈥?use row 1 (row 0 has a height, use different row to avoid confusion)
                ...
                // WB-BE: Shared strings 鈥?write same string "X" in 10 cells
                for (int i = 0; i < 10; i++) {
            ...
            try (org.apache.poi.ss.usermodel.Workbook poiWb = WorkbookFactory.create(new File(path))) {
                ...
                // WB-B9: Column width 鈥?POI uses character-width units; check it's non-default
                // POI width 20 chars 鈥?just verify non-zero
            ...
            // WB-BF: Non-shared strings test 鈥?save with default (shared strings enabled by default)
            ...
            // Check if SaveOptions supports this 鈥?it's in scope but let's verify the default works
