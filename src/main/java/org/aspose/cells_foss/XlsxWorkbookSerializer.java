...
/**
 * Serializer for XLSX workbook files 鈥?thin coordinator that delegates to helper classes.
 */
public final class XlsxWorkbookSerializer {
    ...
    public static void save(WorkbookModel model, OutputStream stream, SaveOptions options) throws IOException {
        ...
        // expect the part to be present, and cells are interned while sheets are built 鈥?
        ...
        // Pre-pass: intentional reset 鈥?counters are accumulated per-sheet in the build loop below
        chartCounter[0] = 1;
        imageCounter[0] = 1;
        ...
        try (ZipOutputStream zip = new ZipOutputStream(stream, StandardCharsets.UTF_8)) {
            ...
            // External links 鈥?preserve verbatim so [1],[2]鈥?references in defined names resolve correctly
            ...
            for (int i = 0; i < sheets.size(); i++) {
                ...
                // Tables 鈥?indices were already assigned by buildTablePartsSnippet
    ...
    private static byte[] workbookXml(WorkbookModel model) {
        ...
        sb.append("</sheets>");
        // External references 鈥?required so [1],[2]鈥?in defined name formulas resolve to the link files
        ...
        for (Map.Entry<Integer, List<Map.Entry<CellAddress, CellRecord>>> rowEntry : byRow.entrySet()) {
            ...
            for (Map.Entry<CellAddress, CellRecord> ce : rowCells) {
                ...
                if (rec.getFormula() != null && !rec.getFormula().isEmpty()) {
                    ...
                    // blank cell with non-default style 鈥?emit as styled empty cell
    ...
    public static WorkbookModel load(InputStream stream, LoadOptions options, LoadDiagnostics diagnostics) throws IOException {
        ...
        // workbookPr 鈥?date1904 + workbook properties
        ...
        // workbook.xml.rels 鈥?rId 鈫?target
        ...
        // xl/externalLinks/ 鈥?preserve verbatim so [1],[2]鈥?references in defined names survive round-trip
        for (int i = 1; ; i++) {
        ...
        // xl/theme/theme1.xml 鈥?preserve verbatim so theme-based fill/font colors round-trip correctly
    ...
    private static void parseWorksheet(WorksheetModel ws, byte[] bytes, String[] sst,
        ...
        for (int i = 0; i < rowNodes.getLength(); i++) {
            ...
            for (int j = 0; j < cNodes.getLength(); j++) {
                ...
                if (!fText.isEmpty()) {
                    ...
                    // Error cell (e.g. #N/A, #VALUE!) 鈥?preserve verbatim without shared-string lookup
        ...
        // Comments 鈥?two passes: first load comment text, then update visibility from VML
        byte[] vmlBytes = null;
        ...
        // Tables 鈥?find via <tableParts> elements then resolve rels
